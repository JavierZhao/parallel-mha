...
int summa(int argc, char *argv[], int rowsA, int colsA, int colsB, int panel_width, int num_comm_row, int num_comm_col, double* A, double* B, double* C)
{
    ...
    // MPI init comm cart
    // MPI_Init(&argc, &argv);
    // printf("MPI initialized in summa\n");
    MPI_Comm parent_comm;
    MPI_Comm_get_parent(&parent_comm);  // Get the parent communicator

    if (parent_comm == MPI_COMM_NULL) {
        fprintf(stderr, "No parent process.\n");
        exit(1);
    }
    ...
    // master code starts
    if ( me == MASTER){
        MPI_Recv(a, rowsA * colsA, MPI_DOUBLE, 0, 0, parent_comm, MPI_STATUS_IGNORE);
        MPI_Recv(b, colsA * colsB, MPI_DOUBLE, 0, 0, parent_comm, MPI_STATUS_IGNORE);
        MPI_Recv(c, rowsA * colsB, MPI_DOUBLE, 0, 0, parent_comm, MPI_STATUS_IGNORE);
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 8) {  // Ensure all needed arguments are passed
        fprintf(stderr, "Usage: %s rowsA colsA colsB panel_width num_comm_row num_comm_col\n", argv[0]);
        MPI_Abort(MPI_COMM_WORLD, 1);
    }

    // Parse the arguments passed from the spawning process
    // print the arguments
    // for (int i = 0; i < argc; i++) {
    //     printf("argv[%d]=%s\n", i, argv[i]);
    // }
    int rowsA = atoi(argv[2]);
    int colsA = atoi(argv[3]);
    int colsB = atoi(argv[4]);
    int panel_width = atoi(argv[5]);
    int num_comm_row = atoi(argv[6]);
    int num_comm_col = atoi(argv[7]);

    // Allocate memory for matrices A, B, and C
    double *A = (double *)malloc(rowsA * colsA * sizeof(double));
    double *B = (double *)malloc(colsA * colsB * sizeof(double));
    ...
    // printf("panel_width=%d, num_comm_row=%d, num_comm_col=%d\n", panel_width, num_comm_row, num_comm_col);

    // printf("Calling summa\n");
