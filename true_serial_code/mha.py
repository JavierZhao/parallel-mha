...
x = torch.randn(
    batch_size, seq_length, embed_dim, dtype=torch.float64
...
save_array_to_csv(Wq, "input/Wq.csv")
save_array_to_csv(Wk, "input/Wk.csv")
save_array_to_csv(Wv, "input/Wv.csv")
save_array_to_csv(Wo, "input/Wo.csv")
save_array_to_csv(bq[:, None].T, "input/bq.csv")  # Transpose to make it a single row
save_array_to_csv(bk[:, None].T, "input/bk.csv")
save_array_to_csv(bv[:, None].T, "input/bv.csv")
save_array_to_csv(bo[:, None].T, "input/bo.csv")
save_array_to_csv(x.detach().numpy().squeeze(), "input/mha_input.csv")
save_array_to_csv(output.detach().numpy().squeeze(), "input/mha_output.csv")
